    ...
    watch = function (obj, name) {
        return new Proxy(obj, {
                ...
                if (name)
                    if (
                        ...
                        if (typeof val === 'function') {
                            console.log(`取值:`, name, '.', p, '=>', 'function');
                        } else {
            ...
            set(target, p, value, receiver) {
                ...
                if (typeof value === 'function') {
                    console.log(`设置值:${name}.${p}=>function`);
                } else {
                    console.log(`设置值:${name}.${p}=>`, value);
                    if (name === '环境数组' && p === '4') {
                        debugger;
                    }
                }
            ...
            has(target, key) {
                console.log(`检查属性存在性: ${name}.${key.toString()}`);
                return key in target;
            },
            ownKeys(target) {
                console.log(`ownKeys检测: ${name}`);
...
function makeFunction(name) {
    ...
    var func = new Function(`
        return function ${name}() {
            console.log('函数传参.${name}', arguments);
        }
    `)();
